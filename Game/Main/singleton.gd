...
func loadConf():
	...
	if ( err == OK) :
		libs.logd( "MSG: loadConf config loaded successfully")
	else :
		libs.logd( "WRN: loadConf failed to open config")
...
func loadRes(path):
	if (path==null) or (path.is_rel_path()):
		libs.logd(str("ERR: loadRes path is null or relative ",path))
		return null
...
func logd(text = "ERR:",reset = false):
